...
def _mcq_kb(question_id: int, options):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(opt["option_text"], callback_data=f"ans_{question_id}_opt_{opt['id']}")]
        for opt in options
    ])
...
def _ranking_kb(question_id: int, remaining_options):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(opt["option_text"], callback_data=f"rank_{question_id}_{opt['id']}")]
...
async def _send_question(query, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if qtype == "likert":
        ...
        options = db.get_options_by_question(q_id)
        context.user_data["sq_rank_all"] = {str(o["id"]): o["option_text"] for o in options}
...
async def select_survey_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not questions:
        await query.edit_message_text("⚠️ This survey has no questions yet. Choose another:")
        return SELECT_SURVEY
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Let's Start!", callback_data="survey_start_yes")],
