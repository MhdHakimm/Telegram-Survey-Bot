...
if st.session_state.mode == "edit":
    ...
    target_q = st.number_input("If answered, skip to Question # (0 for none)", min_value=0, value=0)
    if target_q > 0:
    ...
    for i, q in enumerate(st.session_state.questions):
        with st.expander(f"Q{i+1}: {q['text']} ({q['type'].upper()})"):
            ...
            with col1:
                q["text"] = st.text_input(f"Edit Text Q{i+1}", value=q["text"], key=f"edit_text_{i}")
                
                # Dynamic fields based on type
                if q["type"] == "ranking":
                    q["meta"]["num_items"] = st.slider(f"Options for Q{i+1}", 2, 5, q["meta"].get("num_items", 3), key=f"rank_{i}")
                
                if q["type"] == "likert":
                    q["meta"]["scale"] = st.slider(f"Scale for Q{i+1}", 3, 6, q["meta"].get("scale", 3), key=f"likert_{i}")
                
                if q["type"] == "image":
                    q["meta"]["image_url"] = st.text_input(f"Image URL Q{i+1}", value=q["meta"].get("image_url", ""), key=f"img_{i}")

                # Inline Skip Logic Edit
                q["meta"]["skip_to"] = st.number_input(f"Skip to Q# (Current: Q{i+1})", value=q["meta"].get("skip_to", 0), key=f"skip_{i}")
    ...
    for i, q in enumerate(st.session_state.questions):
        ...
        if q["type"] == "mcq":
            st.radio("Select one", q["meta"].get("options", ["No options"]), key=f"p_mcq_{i}")
            ...
            for score in range(q["meta"].get("scale", 3)):
                cols[score].button(str(score + 1), key=f"p_likert_{i}_{score}", use_container_width=True)

        elif q["type"] == "text":
            st.text_input("Your answer", key=f"p_text_{i}")

        if q["meta"].get("skip_to", 0) > 0:
            st.caption(f"↳ *Logic: After this, jump to Question {q['meta']['skip_to']}*")
        st.write("")
